...
class Assets {
	 ...
	 * @var string
	 */
	private $scriptHandle = '404-to-301';
	...
	public function __construct( $core ) {
		$this->core         = $core;
		$this->version      = aioseo404To301()->version;
		$this->manifestFile = AIOSEO_404_TO_301_DIR . '/dist/manifest.php';
		$this->isDev        = aioseo404To301()->isDev;

		if ( $this->isDev ) {
			$this->domain = getenv( 'VITE_AIOSEO_404_TO_301_DOMAIN' );
			$this->port   = getenv( 'VITE_AIOSEO_404_TO_301_DEV_PORT' );
		}
	 ...
	 * @return void
	 */
	public function load( $asset, $dependencies = [], $data = null, $objectName = 'aioseo404To301' ) {
	 ...
	 * @return void
	 */
	public function registerJs( $asset, $dependencies = [], $data = null, $objectName = 'aioseo404To301' ) {
	 ...
	 * @return void
	 */
	public function enqueueJs( $asset, $dependencies = [], $data = null, $objectName = 'aioseo404To301' ) {
	...
	private function shouldLoadDev() {
		...
		if (
			! $this->isDev ||
			(
				defined( 'AIOSEO_404_TO_301_LOAD_DEV_SCRIPTS' ) &&
				false === AIOSEO_404_TO_301_LOAD_DEV_SCRIPTS
			)
		) {
	...
	public function skipModuleTag( $handle ) {
		if ( ! aioseo404To301()->helpers->stringContains( $handle, $this->jsHandle( '' ) ) ) {
			return true;
		}

		foreach ( $this->noModuleTag as $tag ) {
			if ( aioseo404To301()->helpers->stringContains( $handle, $tag ) ) {
				return true;
			}
		}
	...
	public function normalizeAssetsHost( $path ) {
		static $paths = [];
		if ( isset( $paths[ $path ] ) ) {
			return apply_filters( 'aioseo_404_to_301_normalize_assets_host', $paths[ $path ] );
		}

		// We need to verify the domain on the $path attribute matches
		// what's in site_url() for our assets or they won't load.
		$siteUrl        = site_url();
		$siteUrlEscaped = aioseo404To301()->helpers->escapeRegex( $siteUrl );
		...
		$host          = aioseo404To301()->helpers->escapeRegex( str_replace( 'www.', '', $siteUrlParsed['host'] ) );
		$scheme        = aioseo404To301()->helpers->escapeRegex( $siteUrlParsed['scheme'] );
	...
	private function basePath() {
		return $this->normalizeAssetsHost( plugins_url( 'dist/assets/', AIOSEO_404_TO_301_FILE ) );
	}
}