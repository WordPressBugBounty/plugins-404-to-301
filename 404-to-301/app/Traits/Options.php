<?php
namespace AIOSEO\FourNotFour\Traits;
...
trait Options {
			...
			if ( $preserveHtml ) {
				if ( is_array( $defaults[ $name ]['value'] ) ) {
					foreach ( $defaults[ $name ]['value'] as $k => $v ) {
						$defaults[ $name ]['value'][ $k ] = aioseo404To301()->helpers->decodeHtmlEntities( $v );
					}
				} else {
					$defaults[ $name ]['value'] = aioseo404To301()->helpers->decodeHtmlEntities( $defaults[ $name ]['value'] );
				}
			}
	...
	public function __set( $name, $value ) {
		...
		aioseo404To301()->core->optionsCache->setOptions( $this->optionsName, $cachedOptions );
	...
	private function allFiltered( $all, $include, $exclude ) {
		if ( ! empty( $include ) ) {
			return array_intersect_ukey(
				$all,
				$include,
				function ( $key1, $key2 ) use ( $include ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( ! empty( $exclude ) ) {
			return array_diff_ukey(
				$all,
				$exclude,
				function ( $key1, $key2 ) use ( $exclude ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
	...
	public function update( $optionsName = null, $defaults = null, $options = null ) {
		...
		aioseo404To301()->core->optionsCache->setDb( $optionsName, $refactored );
	 ...
	 * @param  array                                     $arguments Any arguments needed if this was a method called.
	 * @param  mixed                                     $value     The value if we are setting an option.
	 * @return \AIOSEO\FourNotFour\Options\Options            The options object.
	 */
				...
				if ( $preserveHtml ) {
					if ( is_array( $value['value'] ) ) {
						foreach ( $value['value'] as $k => $v ) {
							$value['value'][ $k ] = aioseo404To301()->helpers->decodeHtmlEntities( $v );
						}
					} else {
						$value['value'] = aioseo404To301()->helpers->decodeHtmlEntities( $value['value'] );
					}
				}
	...
	public function getDbOptions( $optionsName ) {
		$cache = aioseo404To301()->core->optionsCache->getDb( $optionsName );
		if ( empty( $cache ) ) {
			...
			// Set the cache.
			aioseo404To301()->core->optionsCache->setDb( $optionsName, $options );
		}

		return aioseo404To301()->core->optionsCache->getDb( $optionsName );
	}
	 ...
	 * @since 1.0.0
	 *
	 * @return \AIOSEO\FourNotFour\Options\Options The cloned Options object.
	 */
	...
	public function getOriginalInstance() {
		...
		$optionName = aioseo404To301()->helpers->toCamelCase( $class->getShortName() );

		if ( isset( aioseo404To301()->{ $optionName } ) ) {
			return aioseo404To301()->{ $optionName };
		}
