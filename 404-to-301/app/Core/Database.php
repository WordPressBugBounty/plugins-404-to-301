...
class Database {
	 ...
	 */
	public $customTables = [
		'aioseo_404_to_301_cache',
		'aioseo_404_to_301_links',
		'aioseo_404_to_301_link_status',
		'aioseo_404_to_301_notifications',
		'aioseo_404_to_301_posts'
	];
	...
	public function getTablesWithColumns() {
		$tables = aioseo404To301()->core->cache->get( 'db_schema' );
		if ( ! empty( $tables ) ) {
			return $tables;
		}
		...
		foreach ( $schema as $row ) {
			...
			// Only cache tables we own, to keep the cache small. `404_to_301` is in the list because
			// the logs and redirects tables predate this plugin joining AIOSEO and kept their
			// original names - without it tableExists() and columnExists() are blind to them.
			if (
				false === strpos( $tableName, 'aioseo' ) &&
				false === strpos( $tableName, '404_to_301' ) &&
				false === strpos( $tableName, 'actionscheduler' )
			) {
				continue;
			}
		...
		aioseo404To301()->core->cache->update( 'db_schema', $tables, DAY_IN_SECONDS );
	...
	public function orderByRaw( $orderBy ) {
		$this->order = array_merge( $this->order, $orderBy );
	...
	public function acquireLock( $lockName, $timeout = 0 ) {
		...
		if ( $acquired ) {
			// Always release the lock, even if a fatal error takes the request down.
