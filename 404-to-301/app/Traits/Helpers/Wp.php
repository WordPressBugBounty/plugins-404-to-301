...
trait Wp {
	...
	public function getPublicPostTypes( $namesOnly = false, $hasArchivesOnly = false, $rewriteType = false ) {
		...
		foreach ( $postTypeObjects as $postTypeObject ) {
			...
			if ( 'attachment' === $postTypeObject->name ) {
				...
				if ( did_action( 'init' ) ) {
					$postTypeObject->label = __( 'Attachments', '404-to-301' );
				}
			}
		...
		return apply_filters( 'aioseo_404_to_301_public_post_types', $postTypes, $namesOnly, $hasArchivesOnly );
	}
	...
	public function installAddon( $name, $network = false ) {
		...
		set_current_screen( 'toplevel_page_404-to-301' );

		// Prepare variables.
		$url = esc_url_raw(
			add_query_arg(
				[
					'page' => '404-to-301-links'
				],
				admin_url( 'admin.php' )
			)
		);
		...
		$fs = aioseo404To301()->core->fs->noConflict();
		$fs->init( $creds );
	...
	public function canInstall() {
		...
		if ( ! wp_is_file_mod_allowed( 'aioseo_404_to_301_can_install' ) ) {
			return false;
		}
	...
	public function canUpdate() {
		...
		// Determine whether file modifications are allowed.
		if ( ! wp_is_file_mod_allowed( 'aioseo_404_to_301_can_update' ) ) {
			return false;
		}
