...
class Doctor extends Command {
	...
	public function __invoke( array $args, array $assoc ): void {
		...
		if ( 'report' === $format ) {
			$this->renderReport( $groups );
		} else {
			$this->renderMachine( $groups, $format );
		}
	...
	private function checkCron(): array {
		...
		foreach ( $cron as $timestamp => $hooks ) {
			foreach ( (array) $hooks as $hook => $ignored ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
				if ( ! $this->prefixMatch( (string) $hook, $prefixes ) ) {
					continue;
				}
				...
				$found[] = [
					...
					'message' => sprintf(
						...
						$this->relativeTime( (int) $timestamp )
					),
				];
			}
		}
	...
	private function checkSettings(): array {
		...
		$emailEnabled = (bool) $options->notifications->email->enabled;
		$recipients   = (array) $options->notifications->email->recipients;
		...
		$redirectEnabled = (bool) $options->redirects->enabled;
		$target          = (string) $options->redirects->target;

		if ( $redirectEnabled ) {
			if ( 'link' === $target ) {
				$url = (string) $options->redirects->link;
				if ( '' === $url ) {
					// Empty is the shipped default and means "the home page" — not a misconfiguration.
					$checks[] = [
						'status'  => self::PASS,
						'message' => sprintf( 'Default redirect URL is unset, so 404s go to the home page (%s).', home_url( '/' ) ),
					];
				...
				$pageId = (int) $options->redirects->pageId;
				if ( $pageId <= 0 ) {
					$checks[] = [
						'status'  => self::FAIL,
					...
					if ( ! $post || 'publish' !== $post->post_status ) {
						$checks[] = [
							'status'  => self::FAIL,
							'message' => sprintf( 'Default redirect target page #%d does not exist or is not published.', $pageId ),
						];
					} else {
						$checks[] = [
							'status'  => self::PASS,
							'message' => sprintf( 'Default redirect target page #%d resolves (%s).', $pageId, get_the_title( $post ) ),
						];
					}
				}
			}
	...
	private function checkDatabase(): array {
		...
		// Tables exist.
		foreach ( [ $logs, $redirects ] as $table ) {
	 ...
	 * @return void
	 */
	private function renderReport( array $groups ): void {
		$summary = [
			self::PASS => 0,
			self::WARN => 0,
			self::FAIL => 0,
	 ...
	 * @return void
	 */
	private function renderMachine( array $groups, string $format ): void {
		$rows = [];
		...
		$this->printRows( [ 'format' => $format ], $rows, [ 'group', 'status', 'message' ] );
	}
	 ...
	 * @return bool
	 */
	private function prefixMatch( string $candidate, array $prefixes ): bool {
