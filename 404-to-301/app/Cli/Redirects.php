...
/**
 * List, inspect, create, update and delete custom redirects.
 *
 * @since   4.0.0
...
class Redirects extends Command {
	...
	public function list( array $args, array $assoc ): void {
		...
		$result = RedirectsModel::paginate( $query );
		...
		$this->printRows( $assoc, $rows, [ 'id', 'source', 'match', 'target', 'type', 'active', 'hits' ] );
	...
	public function update( array $args, array $assoc ): void {
		...
		foreach ( [ 'source', 'target', 'type', 'match-type', 'active', 'notes' ] as $key ) {
			if ( ! isset( $assoc[ $key ] ) ) {
				continue;
			}
	...
	public function delete( array $args, array $assoc ): void {
		if ( ! empty( $assoc['all'] ) ) {
			...
			$count = RedirectsModel::deleteWhere();
			WP_CLI::success(
				sprintf(
					/* translators: %d: rows */
