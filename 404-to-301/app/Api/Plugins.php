...
class Plugins {
	...
	public static function installPlugins( $request ) {
		...
		$error   = esc_html__( 'Installation failed. Please check permissions and try again.', '404-to-301' );
		...
		if ( ! aioseo404To301()->helpers->canInstall() ) {
			return new \WP_REST_Response(
				[
					'success' => false,
					'message' => $error
		...
		foreach ( $plugins as $plugin ) {
			...
			$result = aioseo404To301()->helpers->installAddon( $plugin['plugin'], $network );
			if ( ! $result ) {
	...
	public static function deactivatePlugins( $request ) {
		...
		$plugins = ! empty( $body['plugins'] ) ? $body['plugins'] : [];
		$network = ! empty( $body['network'] ) ? $body['network'] : false;
		$error   = esc_html__( 'Deactivation failed. Please check permissions and try again.', '404-to-301' );
