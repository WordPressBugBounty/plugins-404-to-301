...
/**
 * List, inspect, delete and prune 404 error log entries.
 *
 * @since   4.0.0
...
class Logs extends Command {
	...
	public function list( array $args, array $assoc ): void {
		$perPage = (int) ( $assoc['per-page'] ?? 20 );
		$page    = (int) ( $assoc['page'] ?? 1 );

		$query = [
			'number'  => max( 1, $perPage ),
		...
		$result = LogsModel::paginate( $query );
		...
		$this->printRows( $assoc, $rows, [ 'id', 'url', 'hits', 'status', 'first', 'last' ] );
	...
	public function get( array $args, array $assoc ): void {
		...
		$this->printRows(
			...
			[ 'id', 'url', 'ref', 'ip', 'ua', 'method', 'hits', 'status', 'created_at', 'updated_at' ]
		);
	}
	...
	public function status( array $args, array $assoc ): void {
		...
		if ( LogsModel::setStatus( $id, $map[ $to ] ) ) {
			WP_CLI::success(
				sprintf(
