...
class Sanitizer {
	 ...
	 * @return string[]
	 */
	public static function emailList( $value ): array {
		$candidates = self::stringList( $value );
		$clean      = [];
	 ...
	 *
	 * Referrers and user agents land in exported CSVs verbatim and are set by the visitor, so an
	 * unauthenticated request can plant `=HYPERLINK(...)` in a file an admin later opens in Excel.
