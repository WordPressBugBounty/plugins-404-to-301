...
/**
 * Wraps the `aioseo_404_to_301_migrations_log` option that tracks
 * per-migration execution state for the {@see MigrationRunner}.
 *
 * The option is stored without autoload — healthy sites short-circuit on
 * `lastSchemaVersion === aioseo404To301()->version` before the runner ever
 * reads the log, so paying autoload overhead on every request would be wasted
 * work.
 *
 * @since 202607
 */
class MigrationLog {
	 ...
	 * @var string
	 */
	private $optionName = 'aioseo_404_to_301_migrations_log';
