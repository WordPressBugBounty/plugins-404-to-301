...
class Model implements \JsonSerializable {
	...
	protected function filter( $key ) {
		...
		$table   = aioseo404To301()->core->db->prefix . $this->table;
		$results = aioseo404To301()->core->db->execute( 'SHOW COLUMNS FROM `' . $table . '`', true );
	...
	protected function transform( $data, $set = false ) {
		...
		foreach ( $this->jsonFields as $field ) {
			if ( isset( $data[ $field ] ) && ! aioseo404To301()->helpers->isJsonString( $data[ $field ] ) ) {
				if ( is_array( $data[ $field ] ) && aioseo404To301()->helpers->isArrayNumeric( $data[ $field ] ) ) {
	...
	/**
	 * Whether the model's table has the given column.
	 *
	 * NOTE: save() stamps `created`/`updated` outside filter()'s column intersection, so tables
	...
	protected function hasColumn( $column ) {
		return array_key_exists( $column, (array) $this->getColumns() );
	}
	...
	public function reset( $id = null ) {
		$id = ! empty( $id ) ? $id : ( isset( $this->{$this->pk} ) ? $this->{$this->pk} : null );
		$this->__construct( $id );
	}
